...
public:
    ...
    /// 委托交易
    virtual void trade(TradeType tradeType, double price, double amount, double lever, bool mirror) = 0;
    /// 关联委托交易
    virtual void tradeRelevance(const SymbolInfo &symbolinfo, TradeType tradeType, double price, double amount) = 0;
    /// 画线
    /// \brief setLine
    /// \param name 名称
    /// \param startTime 开始时间，0为起始位置
    /// \param endTime 结束时间，0为结束位置
    /// \param price 价格
    /// \param color 颜色
    /// \param style 1实线，2虚线
    ///
    virtual void setLine(const std::string &name, long long startTime, long long endTime, double price, long color, int style=1) = 0;
    /// 删除画线，name为空删除所有画线
    virtual void delLine(const std::string &name) = 0;
